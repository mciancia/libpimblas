...
int gemv(uint32_t m, uint32_t n, const int *mat, const int *vec, int *out) {
  ...
  uint32_t idx;
  DPU_FOREACH(dpu_set, dpu, idx) { DPU_ASSERT(dpu_prepare_xfer(dpu, &mat[idx * chunk_len])); }
  ...
  DPU_FOREACH(dpu_set, dpu, idx) { DPU_ASSERT(dpu_prepare_xfer(dpu, &out[idx * rows_per_dpu])); }
